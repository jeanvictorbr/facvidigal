...
module.exports = {
    ...
    async execute(interaction) {
        ...
        const components = [];
        // Adiciona o botão de excluir APENAS se houver punições para excluir
        if (punishments.length > 0) {
            const deleteButtonRow = new ActionRowBuilder()
                .addComponents(
                    new ButtonBuilder()
                        .setCustomId(`justica_delete_record_start_${targetUserId}`) // Passa o ID do usuário
                        .setLabel('Excluir um Registro')
                        .setStyle(ButtonStyle.Danger)
                        .setEmoji('🗑️')
                );
            components.push(deleteButtonRow);
        }
